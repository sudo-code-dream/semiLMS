...
import { FileText, Video, Paperclip, Loader2 } from "lucide-react";
import { notFound } from 'next/navigation';


const FileIcon = ({ fileType }: { fileType: 'pdf' | 'video' | 'attachment' | string }) => {
  switch (fileType) {
    case 'pdf':
...
export default function QuarterPage() {
  ...
  const quarterValue = typeof params?.quarter === 'string' ? parseInt(params.quarter) : null;
  ...
  return (
    <div className="h-full p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Quarter {quarter} Study Materials</h1>
      </div>
